/**
 * Renovação Servidor 1 - 3 Meses CORRETO
 * 
...
async function renovar3MesesServidor1(clienteId) {
    console.log('🎯 RENOVAÇÃO SERVIDOR 1 - 3 MESES CORRETO');
    console.log(`🎯 Cliente: ${clienteId}`);
    console.log(`📅 Período: 3 meses (PLANO COMPLETO)`);
    ...
    try {
        ...
        if (renewBtn) {
            ...
            // Selecionar período de 3 meses se necessário
            log('Procurando seleção de período...');
            const periodSelect = await page.$('select[name="periodo"]');
            if (periodSelect) {
                await periodSelect.select('3');
                await sleep(2000);
                log('Período de 3 meses selecionado!', 'success');
            }
            ...
            for (const btn of periodBtns) {
                ...
                if (text && (text.includes('3 mês') || text.includes('3 month') || text.includes('trimestral'))) {
                    await btn.click();
                    await sleep(1000);
        ...
        console.log('');
        console.log('🎉 RENOVAÇÃO DE 3 MESES SERVIDOR 1 CONCLUÍDA!');
        console.log(`🎯 Cliente: ${clienteId}`);
        console.log(`📅 Período: 3 meses (PLANO COMPLETO)`);
...
if (!clienteId) {
    ...
    console.log('📖 Uso: node renovar-servidor1-3meses-correto.cjs <cliente_id>');
    console.log('📖 Exemplo: node renovar-servidor1-3meses-correto.cjs 648718886');
    process.exit(1);
}

// Executar renovação
renovar3MesesServidor1(clienteId).catch(console.error);
