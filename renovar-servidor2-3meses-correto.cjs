/**
 * Renovação Servidor 2 - 3 Meses CORRETO
 * 
 * Versão corrigida que seleciona especificamente "PLANO COMPLETO - TRIMESTRAL"
 * e verifica a mensagem "3 créditos serão deduzidos"
 */
...
async function renovar3MesesCorreto(clienteId) {
    console.log('🎯 RENOVAÇÃO SERVIDOR 2 - 3 MESES CORRETO');
    console.log(`🎯 Cliente: ${clienteId}`);
    console.log(`📅 Período: 3 meses (PLANO COMPLETO - TRIMESTRAL)`);
    console.log(`🔍 Verificação: "3 créditos serão deduzidos"`);
    ...
    try {
        ...
        if (userField && passField) {
            ...
            }, 'tropicalplay', await page.evaluateHandle(() => document.activeElement));
            await sleep(100);
            ...
            await page.evaluate((text, element) => {
                element.value = text;
                element.dispatchEvent(new Event('input', { bubbles: true }));
            }, 'Virginia13', await page.evaluateHandle(() => document.activeElement));
            await sleep(100);
        ...
        if (renewBtn) {
            ...
            // SELEÇÃO CORRETA DO PLANO DE 3 MESES
            log('🎯 INICIANDO SELEÇÃO CORRETA DO PLANO DE 3 MESES', 'info');
            ...
            if (packageDropdown) {
                ...
                log(`Navegando de ${planoAtual} para TRIMESTRAL...`, 'info');
                ...
                const posicaoDestino = posicoes['TRIMESTRAL']; // Posição 4
                
                if (posicaoAtual > posicaoDestino) {
                    // Navegar para cima
                    const navegacoes = posicaoAtual - posicaoDestino;
                    log(`Navegando ${navegacoes} posições para cima (${planoAtual} → TRIMESTRAL)...`, 'info');
                    ...
                    log(`Navegando ${navegacoes} posições para baixo (${planoAtual} → TRIMESTRAL)...`, 'info');
                    ...
                    log('Cliente já está no plano TRIMESTRAL!', 'info');
                }
                ...
                if (opcaoAtual.includes('TRIMESTRAL')) {
                    log('✅ Opção TRIMESTRAL encontrada! Confirmando seleção...', 'success');
                    await page.keyboard.press('Enter');
                    await sleep(2000);
                    
                    // VERIFICAR MENSAGEM DE 3 CRÉDITOS
                    log('🔍 Verificando mensagem "3 créditos serão deduzidos"...', 'verify');
                    await sleep(2000);
                    
                    const mensagem3Creditos = await page.evaluate(() => {
                        const elementos = document.querySelectorAll('*');
                        for (const el of elementos) {
                            const texto = el.textContent || '';
                            if (texto.includes('3 créditos serão deduzidos') || 
                                texto.includes('3 créditos') ||
                                (texto.includes('créditos serão deduzidos') && texto.includes('3'))) {
                                return texto.trim();
                            }
                        }
                        return null;
                    ...
                    if (mensagem3Creditos) {
                        log(`✅ CONFIRMAÇÃO ENCONTRADA: "${mensagem3Creditos}"`, 'success');
                        log('✅ PLANO DE 3 MESES SELECIONADO CORRETAMENTE!', 'success');
                    } else {
                        log('⚠️ Mensagem de 3 créditos não encontrada', 'warning');
                    ...
                    // Tentar encontrar TRIMESTRAL diretamente
                    const opcoes = await page.$$('.el-select-dropdown__item');
                    for (let i = 0; i < opcoes.length; i++) {
                        const texto = await page.evaluate(el => el.textContent?.trim(), opcoes[i]);
                        if (texto && texto.includes('TRIMESTRAL')) {
                            log(`Encontrado TRIMESTRAL na posição ${i}: "${texto}"`, 'success');
        ...
        console.log('');
        console.log('🎉 RENOVAÇÃO DE 3 MESES CONCLUÍDA!');
        console.log(`🎯 Cliente: ${clienteId}`);
        console.log(`📅 Período: 3 meses (PLANO COMPLETO - TRIMESTRAL)`);
        console.log(`✅ Verificação: Mensagem de 3 créditos`);
...
if (!clienteId) {
    ...
    console.log('📖 Uso: node renovar-servidor2-3meses-correto.cjs <cliente_id>');
    console.log('📖 Exemplo: node renovar-servidor2-3meses-correto.cjs 359503850');
    process.exit(1);
}

// Executar renovação
renovar3MesesCorreto(clienteId).catch(console.error);
