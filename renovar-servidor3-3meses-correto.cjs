/**
 * Renovação Servidor 3 - 3 Meses CORRETO
 * 
...
async function renovar3MesesServidor3(clienteId) {
    console.log('🎯 RENOVAÇÃO SERVIDOR 3 - 3 MESES CORRETO');
    console.log(`🎯 Cliente: ${clienteId}`);
    console.log(`📅 Período: 3 meses (3 MÊS C/ ADULTO)`);
    ...
    try {
        ...
        if (renewBtn) {
            ...
            // SELEÇÃO DO PLANO DE 3 MESES
            log('🎯 INICIANDO SELEÇÃO DO PLANO DE 3 MESES', 'info');
            ...
            if (packageDropdown) {
                ...
                const posicaoDestino = posicoes['TRIMESTRAL']; // Posição 2
                ...
                if (opcaoAtual.includes('3 MÊS') || opcaoAtual.includes('TRIMESTRAL') || opcaoAtual.includes('3') || opcaoAtual === 'Não encontrada') {
                    ...
                    // Tentar encontrar "3 MÊS C/ ADULTO" diretamente
                    ...
                    for (let i = 0; i < opcoes.length; i++) {
                        ...
                        if (texto && (texto.includes('3 MÊS C/ ADULTO') || 
                                     texto.includes('TRIMESTRAL') || 
                                     texto.includes('3 MÊS') || 
                                     texto.includes('3'))) {
                            log(`Encontrado 3 MÊS na posição ${i}: "${texto}"`, 'success');
        ...
        console.log('');
        console.log('🎉 RENOVAÇÃO DE 3 MESES SERVIDOR 3 CONCLUÍDA!');
        console.log(`🎯 Cliente: ${clienteId}`);
        console.log(`📅 Período: 3 meses (3 MÊS C/ ADULTO)`);
...
if (!clienteId) {
    ...
    console.log('📖 Uso: node renovar-servidor3-3meses-correto.cjs <cliente_id>');
    console.log('📖 Exemplo: node renovar-servidor3-3meses-correto.cjs 648718886');
    process.exit(1);
}

// Executar renovação
renovar3MesesServidor3(clienteId).catch(console.error);
