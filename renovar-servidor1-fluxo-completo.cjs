/**
 * Renovação Servidor 1 - Fluxo Completo
 * 
...
async function renovarServidor1(clienteId, meses) {
    console.log('🎯 RENOVAÇÃO SERVIDOR 1 - FLUXO COMPLETO');
    ...
    try {
        ...
        if (calendarIcon) {
            ...
            log(`Inserindo quantidade de meses (${meses})...`);
            const monthsField = await page.$('#months');
            if (monthsField) {
                // Limpar campo e inserir quantidade de meses
                await monthsField.click({ clickCount: 3 });
                await monthsField.type(meses.toString(), { delay: 100 });
                await sleep(1000);
                log(`${meses} meses inserido no campo!`, 'success');
            } else {
        ...
        console.log('');
        console.log(`🎉 RENOVAÇÃO DE ${meses} MESES SERVIDOR 1 CONCLUÍDA!`);
        console.log(`🎯 Cliente: ${clienteId}`);
        console.log(`📅 Período: ${meses} meses`);
...
if (!clienteId) {
    ...
    console.log('📖 Uso: node renovar-servidor1-fluxo-completo.cjs <cliente_id> <meses>');
    console.log('📖 Exemplo: node renovar-servidor1-fluxo-completo.cjs 648718886 6');
    process.exit(1);
}
...
// Executar renovação
renovarServidor1(clienteId, meses).catch(console.error);
