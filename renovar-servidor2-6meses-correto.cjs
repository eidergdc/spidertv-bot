/**
 * Renovação Servidor 2 - 6 Meses CORRETO
 * 
 * Versão corrigida que seleciona especificamente "PLANO COMPLETO - SEMESTRAL"
 * e verifica a mensagem "6 créditos serão deduzidos"
 */
...
async function renovar6MesesCorreto(clienteId) {
    console.log('🎯 RENOVAÇÃO SERVIDOR 2 - 6 MESES CORRETO');
    console.log(`🎯 Cliente: ${clienteId}`);
    console.log(`📅 Período: 6 meses (PLANO COMPLETO - SEMESTRAL)`);
    console.log(`🔍 Verificação: "6 créditos serão deduzidos"`);
    ...
    try {
        ...
        if (renewBtn) {
            ...
            // SELEÇÃO CORRETA DO PLANO DE 6 MESES
            log('🎯 INICIANDO SELEÇÃO CORRETA DO PLANO DE 6 MESES', 'info');
            ...
            if (packageDropdown) {
                ...
                log(`Navegando de ${planoAtual} para SEMESTRAL...`, 'info');
                ...
                const posicaoDestino = posicoes['SEMESTRAL']; // Posição 5
                
                if (posicaoAtual > posicaoDestino) {
                    // Navegar para cima
                    const navegacoes = posicaoAtual - posicaoDestino;
                    log(`Navegando ${navegacoes} posições para cima (${planoAtual} → SEMESTRAL)...`, 'info');
                    ...
                    log(`Navegando ${navegacoes} posições para baixo (${planoAtual} → SEMESTRAL)...`, 'info');
                    ...
                    log('Cliente já está no plano SEMESTRAL!', 'info');
                }
                ...
                if (opcaoAtual.includes('SEMESTRAL')) {
                    log('✅ Opção SEMESTRAL encontrada! Confirmando seleção...', 'success');
                    await page.keyboard.press('Enter');
                    await sleep(2000);
                    
                    // VERIFICAR MENSAGEM DE 6 CRÉDITOS
                    log('🔍 Verificando mensagem "6 créditos serão deduzidos"...', 'verify');
                    await sleep(2000);
                    
                    const mensagem6Creditos = await page.evaluate(() => {
                        const elementos = document.querySelectorAll('*');
                        for (const el of elementos) {
                            const texto = el.textContent || '';
                            if (texto.includes('6 créditos serão deduzidos') || 
                                texto.includes('6 créditos') ||
                                texto.includes('créditos serão deduzidos')) {
                                return texto.trim();
                            }
                        }
                        return null;
                    ...
                    if (mensagem6Creditos) {
                        log(`✅ CONFIRMAÇÃO ENCONTRADA: "${mensagem6Creditos}"`, 'success');
                        log('✅ PLANO DE 6 MESES SELECIONADO CORRETAMENTE!', 'success');
                    } else {
                        log('⚠️ Mensagem de 6 créditos não encontrada', 'warning');
                    ...
                    // Tentar encontrar SEMESTRAL diretamente
                    const opcoes = await page.$$('.el-select-dropdown__item');
                    for (let i = 0; i < opcoes.length; i++) {
                        const texto = await page.evaluate(el => el.textContent?.trim(), opcoes[i]);
                        if (texto && texto.includes('SEMESTRAL')) {
                            log(`Encontrado SEMESTRAL na posição ${i}: "${texto}"`, 'success');
        ...
        console.log('');
        console.log('🎉 RENOVAÇÃO DE 6 MESES CONCLUÍDA!');
        console.log(`🎯 Cliente: ${clienteId}`);
        console.log(`📅 Período: 6 meses (PLANO COMPLETO - SEMESTRAL)`);
        console.log(`✅ Verificação: Mensagem de 6 créditos`);
...
if (!clienteId) {
    ...
    console.log('📖 Uso: node renovar-servidor2-6meses-correto.cjs <cliente_id>');
    console.log('📖 Exemplo: node renovar-servidor2-6meses-correto.cjs 359503850');
    process.exit(1);
}

// Executar renovação
renovar6MesesCorreto(clienteId).catch(console.error);
