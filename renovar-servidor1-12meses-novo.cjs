...
async function renovar12MesesServidor1(clienteId) {
    ...
    try {
        ...
        log('Configurando período de 12 meses...');
        const monthsInput = await page.$('#months');
        if (monthsInput) {
            await monthsInput.click();
            await monthsInput.evaluate(input => input.select());
            await monthsInput.type('12', { delay: 100 });
            await sleep(1000);
            log('Período de 12 meses configurado!', 'success');
        }
...
if (!clienteId) {
    ...
    console.log('📖 Uso: node renovar-servidor1-12meses-novo.cjs <cliente_id>');
    console.log('📖 Exemplo: node renovar-servidor1-12meses-novo.cjs 648718886');
    process.exit(1);
}
