...
async function renovar6MesesServidor1(clienteId) {
    ...
    console.log(`📅 Período: 6 meses (PLANO COMPLETO)`);
    ...
    try {
        ...
        if (renewBtn) {
            ...
            // Selecionar período de 6 meses se necessário
            log('Procurando seleção de período...');
            const periodSelect = await page.$('select[name="periodo"]');
            if (periodSelect) {
                await periodSelect.select('6');
                await sleep(2000);
                log('Período de 6 meses selecionado!', 'success');
            }
            ...
            for (const btn of periodBtns) {
                ...
                if (text && (text.includes('6 mês') || text.includes('6 month') || text.includes('semestral'))) {
                    await btn.click();
                    await sleep(1000);
...
if (!clienteId) {
    ...
    console.log('📖 Uso: node renovar-servidor1-6meses-correto.cjs <cliente_id>');
    console.log('📖 Exemplo: node renovar-servidor1-6meses-correto.cjs 648718886');
    process.exit(1);
}
