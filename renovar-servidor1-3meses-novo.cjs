...
async function renovar3MesesServidor1(clienteId) {
    ...
    try {
        ...
        log('Configurando período de 3 meses...');
        const monthsInput = await page.$('#months');
        if (monthsInput) {
            await monthsInput.click();
            await monthsInput.evaluate(input => input.select());
            await monthsInput.type('3', { delay: 100 });
            await sleep(1000);
            log('Período de 3 meses configurado!', 'success');
        }
...
if (!clienteId) {
    ...
    console.log('📖 Uso: node renovar-servidor1-3meses-novo.cjs <cliente_id>');
    console.log('📖 Exemplo: node renovar-servidor1-3meses-novo.cjs 648718886');
    process.exit(1);
}
