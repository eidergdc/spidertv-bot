/**
 * Renovação Servidor 3 - 6 Meses CORRETO
 * 
...
async function renovar6MesesServidor3(clienteId) {
    console.log('🎯 RENOVAÇÃO SERVIDOR 3 - 6 MESES CORRETO');
    console.log(`🎯 Cliente: ${clienteId}`);
    console.log(`📅 Período: 6 meses (PLANO SEMESTRAL)`);
    ...
    try {
        ...
        if (renewBtn) {
            ...
            // SELEÇÃO DO PLANO DE 6 MESES
            log('🎯 INICIANDO SELEÇÃO DO PLANO DE 6 MESES', 'info');
            ...
            if (packageDropdown) {
                ...
                const posicaoDestino = posicoes['SEMESTRAL']; // Posição 4
                ...
                if (opcaoAtual.includes('SEMESTRAL') || opcaoAtual.includes('6') || opcaoAtual === 'Não encontrada') {
                    ...
                    const mensagem6Creditos = await page.evaluate(() => {
                        ...
                        for (const el of elementos) {
                            ...
                            if (texto.includes('6 créditos serão deduzidos') || 
                                ...
                                (texto.includes('créditos serão deduzidos') && texto.includes('6'))) {
                                return texto.trim();
                            }
                        }
                        return null;
                    ...
                    // Tentar encontrar "6 MÊS C/ ADULTO" diretamente
                    ...
                    for (let i = 0; i < opcoes.length; i++) {
                        ...
                        if (texto && (texto.includes('6 MÊS C/ ADULTO') || 
                                     texto.includes('SEMESTRAL') || 
                                     texto.includes('6 MÊS') || 
                                     texto.includes('6'))) {
                            log(`Encontrado 6 MÊS na posição ${i}: "${texto}"`, 'success');
        ...
        console.log('');
        console.log('🎉 RENOVAÇÃO DE 6 MESES SERVIDOR 3 CONCLUÍDA!');
        console.log(`🎯 Cliente: ${clienteId}`);
        console.log(`📅 Período: 6 meses (PLANO SEMESTRAL)`);
...
if (!clienteId) {
    ...
    console.log('📖 Uso: node renovar-servidor3-6meses-correto.cjs <cliente_id>');
    console.log('📖 Exemplo: node renovar-servidor3-6meses-correto.cjs 648718886');
    process.exit(1);
}

// Executar renovação
renovar6MesesServidor3(clienteId).catch(console.error);
