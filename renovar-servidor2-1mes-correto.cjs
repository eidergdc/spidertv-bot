/**
 * Renovação Servidor 2 - 1 Mês CORRETO
 * 
 * Versão corrigida que seleciona especificamente "PLANO COMPLETO" (mensal)
 * e verifica a mensagem "1 crédito será deduzido"
 */
...
async function renovar1MesCorreto(clienteId) {
    console.log('🎯 RENOVAÇÃO SERVIDOR 2 - 1 MÊS CORRETO');
    ...
    try {
        ...
        if (renewBtn) {
            ...
            // SELEÇÃO CORRETA DO PLANO DE 1 MÊS
            log('🎯 INICIANDO SELEÇÃO CORRETA DO PLANO DE 1 MÊS', 'info');
            ...
            if (packageDropdown) {
                ...
                const posicaoDestino = posicoes['MENSAL']; // Posição 1
                ...
                if (opcaoAtual.includes('PLANO COMPLETO') && !opcaoAtual.includes('TRIMESTRAL') && !opcaoAtual.includes('SEMESTRAL') && !opcaoAtual.includes('ANUAL') && !opcaoAtual.includes('BIMESTRAL')) {
                    log('✅ Opção MENSAL encontrada! Confirmando seleção...', 'success');
                    await page.keyboard.press('Enter');
                    await sleep(2000);
                    ...
                    // Tentar encontrar MENSAL diretamente
                    const opcoes = await page.$$('.el-select-dropdown__item');
                    for (let i = 0; i < opcoes.length; i++) {
                        const texto = await page.evaluate(el => el.textContent?.trim(), opcoes[i]);
                        if (texto && texto.includes('PLANO COMPLETO') && 
                            !texto.includes('TRIMESTRAL') && !texto.includes('SEMESTRAL') && 
                            !texto.includes('ANUAL') && !texto.includes('BIMESTRAL')) {
                            log(`Encontrado MENSAL na posição ${i}: "${texto}"`, 'success');
        ...
        console.log('');
        console.log('🎉 RENOVAÇÃO DE 1 MÊS CONCLUÍDA!');
        console.log(`🎯 Cliente: ${clienteId}`);
        console.log(`📅 Período: 1 mês (PLANO COMPLETO)`);
...
if (!clienteId) {
    ...
    console.log('📖 Uso: node renovar-servidor2-1mes-correto.cjs <cliente_id>');
    console.log('📖 Exemplo: node renovar-servidor2-1mes-correto.cjs 359503850');
    process.exit(1);
}

// Executar renovação
renovar1MesCorreto(clienteId).catch(console.error);
